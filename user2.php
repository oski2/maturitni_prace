<?php
//Uživatelská stránka pro studenty třetího ročníku, vybírají si čtyři semináře a blok
...
<html>
	...
	<body>
		...
		<!--Formulář pro vybírání seminářů a bloku pro druhý ročník-->
		<form action="studentvyber2.php" method="post">
			...
			<p>Vyberte si první seminář
				<select name="s1">
					<?php
					//Smyčka která vytváří možnosti výběru seminářů podle tabulky seminářů
					$sql = "SELECT id, seminar FROM seminare WHERE (rocnik = '3' AND k2 = 1)";
		...
		<?php
					...
					$ukaz = "SELECT seminar FROM seminare INNER JOIN studenti 
					ON (seminare.id = studenti.s1 OR seminare.id = studenti.s2 OR seminare.id = studenti.s3 OR seminare.id = studenti.s4) 
					WHERE studenti.id = '$userID'";
		...
		<?php
		$druhekolo = "SELECT seminar FROM seminare INNER JOIN studenti ON 
					(seminare.id = studenti.s1 OR seminare.id = studenti.s2 OR seminare.id = studenti.s3 OR seminare.id = studenti.s4)
					WHERE (studenti.id = '$userID' AND k2 = 0)";
		$druhekoloquery = mysqli_query($conn, $druhekolo);
		
		if (mysqli_num_rows($druhekoloquery) > 0) {
			$ukaz = "SELECT seminar FROM seminare INNER JOIN studenti ON 
					(seminare.id = studenti.s1 OR seminare.id = studenti.s2 OR seminare.id = studenti.s3 OR seminare.id = studenti.s4) 
					WHERE (studenti.id = '$userID' AND seminare.k2 = 0)";
			$ukazquery = mysqli_query($conn, $ukaz);
		...
		<form action="zmenahesla2.php" method = "post">
